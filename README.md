...
- canvas 库 https://github.com/konvajs/konva 支持react ,支持函数组件,官方 demo 给的还是 class
对应的 react 库为： https://github.com/konvajs/react-konva
对应的文档还可以，api 说明的比较清楚，都有 demo,不过库更新不是太频繁了,star数量 10k

- canvas 库  https://github.com/fabricjs/fabric.js 支持 react
这个库的 demo 还比较多，支持 react 这个库支持 react star数 26k
最终选择使用 fabric 这个库，面向对象写法，感觉不错，功能也强大，还支持 svg 功能

不过在解析 pgm 的时候 Error loading http://localhost:3008/upload/files-1702951513030.pgm

最后发现这两个库都没有canvas 相应的 createImageData 和 getImageData, putImageData
所以这里还是要获取 canvas 的 context 对象，再调用原生的 canvas 完成解析 pgm地图并加载到 canvas 的任务

现在后端的 http://localhost:3008/upload/files-1702951513030.pgm 已经可以在浏览器 get 访问了
现在看看怎么让 canvas 加载远程的 pgm 图片，fabric的 fabric.Image.fromURL 方法不行,不支持 pgm 格式


现在实现一个请求文件并返回数据流 arrayBuffer 的接口，请求接口获取 pgm 的二进制数据，现在想通过 canvas 的原生api ctx.putImageData()方法使用，发现不行

原生 api https://developer.mozilla.org/en-US/docs/Web/API/CanvasRenderingContext2D/createImageData

了解了下 ctx.putImageData() 这里可以传入 ctx.getImageData()的数据（二进制的像素）
也可以自己创建一个 createImageData ，默认返回每个格式为空的数据，然后再对这个数据进行遍历处理把每个格式填充
每个格子是一个 array[4],分别表示 r g b a = 255，都写上数据之后 就可以用putImageData 显示出来了
有点像把这些数据转换成 png 之前 需要处理的一样
------------------------------------------------------------

学习原生 canvas 的相关 api，理解机制

https://developer.mozilla.org/en-US/docs/Web/API/Canvas_API/Tutorial/Drawing_shapes

canvas 创建，画基本图片，矩形和 path 

矩形相关

1. 矩形  ctx.fillRect(25, 25, 100, 100) 填充
2. 矩形 ctx.clearRect(45, 45, 60, 60) 清除（🆑为透明）
3. 矩形 ctx.strokeRect(50,50,50,50) 描边
...
计算机图形学中，坐标原点在左上方，而数学中的笛卡尔坐标则是在左下方，
在 canvas 通过 ctx.translate(0, canvas.height); ctx.scale(1,-1)可以把图形学的坐标转换为笛卡尔坐标
...
-  发现博主使用的是 create.js 这个已经很多年了，不推荐使用
-  https://github.com/Flipboard/react-canvas 这个库也是太久了不好用
...
13. 现在后端的逻辑是上传本地的 pmg 图片到远程，保存到后端某个文件夹内，同时把上传的 pgm 图片的 url 和图片的相关信息比如宽度和高度，上次修改时间记录到数据库，同时保存工程名到数据库 ,其它信息已经存进去了，还有个宽和度的信息没有存进去.
14. 操作区选择有两种形式，一是使用 floatbutton,一种是使用 radio 的平铺填充样式
15. 现在要把 canvas 相关东西调研搞出来，至于怎么放都是一些细节的东西
