...
const pgmXTControl: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const canvas: fabric.Canvas = new fabric.Canvas(canvasEl.current, { x: 0, y: 0 })
    setCanvas(canvas)
    /*
    //设置绘制模式
  ...
  const erase = (e) => {
    ...
    if (isErasing) {
      ...
      canvas
        .getContext('2d')
        .clearRect(x - eraserSize / 2, y - eraserSize / 2, eraserSize, eraserSize)
    }
  }
  const eraserBrushFunc = () => {
    //eraserBrush的功能 fabric 自己没有，自己编译的也有问题，有一个 Npm 包用的5.2.0版本，
    //不过在调用 canvas.dispose() 的时候有一个报错，算了，earser 功能只接使用原生的
    const context = canvas.getContext('2d') as CanvasRenderingContext2D
    //draw
    //context.globalCompositeOperation = 'source-over'
    //earse
    context.globalCompositeOperation = 'destination-out'
    console.log(fabric.version, context.getContextAttributes())
  }
  return (
    <div className="p-2 h-full flex justify-center items-center border-2 border-red-300 border-soli">
      <Button type="primary" onClick={() => (canvas.globalCompositeOperation = 'destination-out')}>
        橡皮擦
      </Button>
      <div className="border-2 min-h-[550px] flex justify-center items-center">
        <canvas
          ...
          width={canvaWidth}
          height={canvasHeight}
          className="border-2 border-yellow-300 border-soli"
